...
def build_silver(bronze_df, entity_config, env_config, entity_name, logger=None, replay =False):
    """
    ...
    logger.info("silver Data Read Successfully from stream file")
    ...
    cdc_df = parse_debezium(bronze_decoded_df, logger=logger, replay=replay)
    logger.info("Finished parsing Debezium events")


    #call map_entity fn to convert cddc even into entity-specific silver records
    logger.info("calling map_entity function to convert cdc event into entity-specific silver records")
    silver_df = map_entity(cdc_df, entity_config,logger=logger, replay=replay)
    logger.info("Running Validation on Silver DataFrame...")
    validated_df = validate(silver_df, entity_config, logger=logger)
